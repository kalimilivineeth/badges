 ...
 var queryChaincode = async function (channelName, chaincodeName, args,
   ...
   let connectionOptions = {
	 ...
	 try {
		 ...
		 logger.log('Connect to Fabric gateway.');
		 await gateway.connect(connectionProfile, connectionOptions);
		 // Access network
		 logger.log(`Use network channel: ${channelName}`);
		 const network = await gateway.getNetwork(channelName);
		 //Get addressability to rta contract
	   logger.log(`Use contract: ${chaincodeName}`);
		 const contract = network.getContract(chaincodeName);

     logger.log(`Before evaluateTransaction args`, args);

		 const results = await contract.evaluateTransaction(functionName, ...args);
     logger.log(`After evaluateTransaction`);
