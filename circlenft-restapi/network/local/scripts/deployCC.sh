...
if [ "$CC_SKIP_PACKAGE_INSTALL" = "true" ]; then
  ...
  if [ "$CC_NAME" = "nftchaincode" ]; then
    # peer0 of org1
    installChaincode 0 org1
	  errorln "Chaincode name should be 'nftchaincode'"
  fi
fi
...
if [ "$CHANNEL_NAME" = "defaultchannel" ]; then
  ...
  queryInstalled 0 org1

  ## approve the definition for org1
  approveForMyOrg 0 org1

  ## commit the definition
  commitChaincodeDefinition org1

  ## query to see that the definition committed successfully
  queryCommitted 0 org1
