...
eventHelper.registerEvent = async function (sendAll) {
    ...
    let { adminUserId } = await helper.getAdminCreds(config.collabera);
	...
	let connectionProfile = await helper.getCCP(config.collabera);
    ...
    try {
        ...
        listener = async (event) => {
            if (event.eventName == 'TRANSFER_ITEM'){
                ...
                // transfer the asset
                nftController.transferNftEvent(asset, adminUserId, config.collabera);
            }
        };
        // start the client side event service and register the listener
        logger.debug(`--> Start contract event stream to peer in ${config.collabera}`);
